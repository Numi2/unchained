...
fn count_epochs(db_dir: &std::path::Path) -> u64 {
    let cfg = unchained::config::Config {
        net: unchained::config::Net { listen_port: 0, bootstrap: vec![], max_peers: 8, connection_timeout_secs: 1, public_ip: None, sync_timeout_secs: 1 },
        ...
        epoch: unchained::config::Epoch { seconds: 222, target_leading_zeros: 1, target_coins_per_epoch: 1, max_coins_per_epoch: 1, retarget_interval: 10 },
