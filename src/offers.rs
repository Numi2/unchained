...
use hyper::{Body, Method, Request as HRequest, Response as HResponse, StatusCode};
use std::sync::Arc;
...
pub async fn serve(cfg: crate::config::Offers, db: Arc<Store>, net: NetHandle) -> Result<()> {
    ...
    tokio::spawn(async move {
        let listener = match StdTcpListener::bind(&bind_addr) {
            Ok(l) => l,
            Err(e) => {
                eprintln!("offers api bind error: {}", e);
                return;
            }
        };
                    ...
                    async move {
                        ...
                        let resp = match (method, path.as_str()) {
                            (Method::GET, "/offers") => {
                                ...
                                if q.contains("stream=1") {
                                    sse_offers(&svc, req).await
                                } else {
                                    get_offers(&svc, req).await
                                }
                            }
                            _ => not_found(),
                        };
        ...
        if let Err(e) = hyper::Server::from_tcp(listener).unwrap().serve(make).await {
            eprintln!("offers api serve error: {}", e);
        }
    });
    Ok(())
}
...
async fn sse_offers(svc: &Arc<OffersService>, req: HRequest<Body>) -> HResponse<Body> {
    ...
    let mut since_millis: u128 = params
        .get("since")
    ...
    if since_millis == 0 {
            ...
            .or_else(|| req.headers().get("Last-Event-ID"))
        {
            if let Ok(s) = h.to_str() {
                since_millis = s.parse::<u128>().unwrap_or(0);
            }
        }
    }
    ...
    let initial_pairs: Vec<(u128, Vec<u8>)> = {
        let cf = if let Some(cf) = svc.db.db.cf_handle("offers") {
            cf
        } else {
            return err_response("offers CF missing", 500);
        };
        if since_millis > 0 {
            ...
            let it = svc.db.db.iterator_cf_opt(
                cf,
                ro,
                rocksdb::IteratorMode::From(
                    &since_millis.to_le_bytes(),
        ...
        for (ts, v) in initial_pairs {
            if let Ok(mut offer) = bincode::deserialize::<crate::wallet::OfferDocV2>(&v) {
                ...
                if let Ok(js) = serde_json::to_vec(&offer) {
                    ...
                    buf.push_str("\n\n");
                    if tx.send_data(Bytes::from(buf)).await.is_err() {
                        return;
                    }
                }
            }
...
async fn get_offers(svc: &Arc<OffersService>, req: HRequest<Body>) -> HResponse<Body> {
    ...
    let since = params
        ...
        .unwrap_or(0);
    let redact_sig = params.get("redact_sig").map(|v| v == "1").unwrap_or(false);
    let cf = match svc.db.db.cf_handle("offers") {
        Some(cf) => cf,
        None => return err_response("offers CF missing", 500),
    };
    ...
    let it = svc.db.db.iterator_cf_opt(
        cf,
        ro,
        rocksdb::IteratorMode::From(&since.to_le_bytes(), rocksdb::Direction::Forward),
    );
...
fn err_response(msg: &str, status: u16) -> HResponse<Body> {
    json_response(&serde_json::json!({"error": msg}), status)
}
fn not_found() -> HResponse<Body> {
