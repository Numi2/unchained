...
use unchained::{config, epoch::Anchor, storage, transfer::Spend};
...
fn main() -> anyhow::Result<()> {
    ...
    if current_latest.num <= rollback_to {
        println!(
            "✅ Already at or below epoch {}. Nothing to do.",
            rollback_to
        );
        return Ok(());
    }
    ...
    if let Ok(Some(bad_epoch)) = db.get::<Anchor>("epoch", &problem_epoch.to_le_bytes()) {
        println!(
            "\n⚠️  Found epoch {} with hash: {}",
            problem_epoch,
            hex::encode(&bad_epoch.hash[..8])
        );
        ...
        if expected != bad_epoch.hash {
            ...
            println!(
                "   Expected parent: {}",
                hex::encode(&rollback_anchor.hash[..8])
            );
            println!(
                "   Computed hash should be: {}",
    ...
    println!(
        "\n⚠️  This will remove epochs {} through {}",
        rollback_to + 1,
        current_latest.num
    );
    ...
    let epoch_cf = db
        .db
        .cf_handle("epoch")
        .ok_or_else(|| anyhow::anyhow!("'epoch' CF not found"))?;
    let anchor_cf = db
        .db
        .cf_handle("anchor")
        .ok_or_else(|| anyhow::anyhow!("'anchor' CF not found"))?;
    let coin_cf = db
        .db
        .cf_handle("coin")
        .ok_or_else(|| anyhow::anyhow!("'coin' CF not found"))?;
    let coin_epoch_cf = db
        .db
        .cf_handle("coin_epoch")
        .ok_or_else(|| anyhow::anyhow!("'coin_epoch' CF not found"))?;
    let selected_cf = db
        .db
        .cf_handle("epoch_selected")
        .ok_or_else(|| anyhow::anyhow!("'epoch_selected' CF not found"))?;
    let leaves_cf = db
        .db
        .cf_handle("epoch_leaves")
        .ok_or_else(|| anyhow::anyhow!("'epoch_leaves' CF not found"))?;
    let spend_cf = db
        .db
        .cf_handle("spend")
        .ok_or_else(|| anyhow::anyhow!("'spend' CF not found"))?;
    let nullifier_cf = db
        .db
        .cf_handle("nullifier")
        .ok_or_else(|| anyhow::anyhow!("'nullifier' CF not found"))?;
    ...
    if let Ok(Some(new_latest)) = db.get::<Anchor>("epoch", b"latest") {
        if new_latest.num == rollback_to {
            ...
            if db
                .get::<Anchor>("epoch", &problem_epoch.to_le_bytes())?
                .is_none()
            {
            ...
            println!(
                "⚠️  Latest epoch is {} (expected {})",
                new_latest.num, rollback_to
            );
        }
    }
    ...
    println!(
        "\nThe node will request and validate the correct epoch {} from the network.",
        problem_epoch
    );
