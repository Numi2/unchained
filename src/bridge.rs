...
use crate::x402;
use crate::{crypto::Address, storage::Store};
...
impl BridgeService {
    ...
    fn pending_submitted_amounts(
        ...
        let cf = self
            .db
            .db
            .cf_handle("bridge_pending")
            .ok_or_else(|| anyhow!("bridge_pending CF missing"))?;
        let iter = self.db.db.iterator_cf(cf, rocksdb::IteratorMode::Start);
        let mut per_addr = 0u64;
        let mut global = 0u64;
        for item in iter {
            let (_, bytes) = item.map_err(|e| anyhow!(e.to_string()))?;
            let op: PendingBridgeOp = bincode::deserialize(&bytes)?;
            if !matches!(op.status, PendingStatus::Submitted) || op.created_at < window_start {
                continue;
            }
            global = global.saturating_add(op.amount);
            if hex::encode(op.from) == from_addr_hex {
                per_addr = per_addr.saturating_add(op.amount);
            }
        }
        Ok((per_addr, global))
    }
    ...
    pub fn list_pending(&self) -> Result<Vec<PendingBridgeOp>> {
        ...
        for item in iter {
            let (_k, val) = item?;
            if let Ok(op) = bincode::deserialize::<PendingBridgeOp>(&val) {
                v.push(op);
            }
        }
        Ok(v)
    ...
    pub fn is_sui_tx_processed(&self, sui_tx_hash: &str) -> Result<bool> {
        let cf = self
            .db
            .db
            .cf_handle("bridge_processed_sui")
            .ok_or_else(|| anyhow!("bridge_processed_sui CF missing"))?;
        Ok(self.db.db.get_cf(cf, sui_tx_hash.as_bytes())?.is_some())
    }
}

fn prune_window(window: &mut VecDeque<(u64, u64)>, window_start: u64) {
...
fn record_locked_amount(
    ...
    let q = st
        .per_addr_window
        .entry(from_addr_hex.to_string())
        .or_insert_with(VecDeque::new);
    prune_window(q, window_start);
    q.push_back((now, amount));
    prune_window(&mut st.global_window, window_start);
    st.global_window.push_back((now, amount));
}
...
pub async fn serve(
                ...
                async move {
                    ...
                    if svc.cfg.x402_enabled && is_protected_path(&svc.cfg, &path) {
                        ...
                        if let Some(h) = req.headers().get(crate::x402::HEADER_X_PAYMENT) {
                            if let Ok(hs) = h.to_str() {
                                if let Ok(()) = verify_x402_any_receipt(&svc, hs, Some(&path)).await
                                {
                                    ...
                                    return Ok::<_, std::convert::Infallible>(err_response(
                        ...
                        if !x402_paid_ok {
                            ...
                            let ch = match build_x402_challenge(&svc, &path) {
                                Ok(c) => c,
                                Err(e) => {
                                    return Ok::<_, std::convert::Infallible>(err_response(
                                        &format!("{}", e),
                    ...
                    let resp = match (method, path.as_str()) {
                        ...
                        (Method::GET, "/402/challenge") => {
                            ...
                            match build_x402_challenge(&svc, &resource) {
                                Ok(ch) => {
                                    let mut r = json_response(&ch, 402);
                                    *r.status_mut() = hyper::StatusCode::PAYMENT_REQUIRED;
                                    r
                                }
                        ...
                        (Method::POST, "/402/verify") => {
                            ...
                            let (receipt_str, resource_opt) =
                                match serde_json::from_slice::<VerifyReq>(&body) {
                                    Ok(v) => (v.receipt, v.resource),
                                    Err(_) => (String::from_utf8_lossy(&body).to_string(), None),
                                };
                            match verify_x402_any_receipt(
                                &svc,
                                &receipt_str,
                                resource_opt.as_deref(),
                            )
                            .await
                            {
                        ...
                        (Method::POST, "/meta-transfer/submit") => {
                            ...
                            match meta_transfer_submit(&svc, &body).await {
                                Ok(hashes) => {
                                    json_response(&serde_json::json!({"spend_hashes": hashes}), 200)
                                }
                        ...
                        (Method::GET, "/bridge_events") => match read_events(&svc.db, 200) {
                        ...
                        (Method::POST, p) if p.starts_with("/admin/bridge_pending_requeue/") => {
                            ...
                            let cf = if let Some(cf) = svc.db.db.cf_handle("bridge_pending") {
                                cf
                            } else {
                                return Ok::<_, std::convert::Infallible>(err_response(
                                    "bridge_pending CF missing",
                                    500,
                                ));
                            };
                            match svc.db.db.get_cf(cf, id.as_bytes()) {
                                Ok(Some(bytes)) => {
                                    if let Ok(mut op) =
                                        bincode::deserialize::<PendingBridgeOp>(&bytes)
                                    {
                                        op.status = PendingStatus::Submitted;
                                        op.last_attempt = 0;
                                        op.retry_count = 0;
                                        if let Ok(ser) = bincode::serialize(&op) {
                                            let _ = svc.db.db.put_cf(cf, id.as_bytes(), &ser);
                                        }
                                        json_response(&serde_json::json!({"ok": true}), 200)
                                    } else {
                                        err_response("malformed pending op", 500)
                                    }
                                }
                        ...
                        (Method::GET, "/get_bridge_status") => match svc.get_status() {
                            Ok(st) => json_response(&st, 200),
                            Err(e) => err_response(&format!("{}", e), 500),
                        },
                        // Stable alias
                        (Method::GET, "/bridge/status") => match svc.get_status() {
                            Ok(st) => json_response(&st, 200),
                            Err(e) => err_response(&format!("{}", e), 500),
                        },
                        ...
                        (Method::GET, "/get_pending_bridge_ops") => match svc.list_pending() {
                        ...
                        (Method::POST, "/bridge_out") => {
                            ...
                            match serde_json::from_slice::<BridgeOutReq>(&whole) {
                                Ok(reqo) => match bridge_out_submit(&svc, reqo).await {
                                    Ok(BridgeOutResult::Locked { tx_hash }) => {
                                        json_response(&SubmitResp { tx_hash }, 200)
                                    }
                                    Ok(BridgeOutResult::Pending { op_id }) => json_response(
                                        &serde_json::json!({"pending": true, "op_id": op_id}),
                                        200,
                                    ),
                                    Err(e) => err_response(&format!("{}", e), 400),
                                },
    ...
    tokio::spawn(async move {
        if let Err(e) = Server::bind(&addr).serve(make).await {
            eprintln!("bridge rpc serve error: {}", e);
        }
    });
        ...
        tokio::spawn(async move {
            loop {
                ...
                if let Some(cf_name) = Some("bridge_pending") {
                    let cf = match svc_h.db.db.cf_handle(cf_name) {
                        Some(c) => c,
                        None => continue,
                    };
                            ...
                            if let Ok(mut op) = bincode::deserialize::<PendingBridgeOp>(&v) {
                                    ...
                                    && op.created_at < cutoff
                                {
                                    if let Some(cf_oc) = svc_h.db.db.cf_handle("bridge_op_coins") {
                                        if svc_h.db.db.get_cf(cf_oc, &k).ok().flatten().is_some() {
                                            continue;
                                        }
                                    }
                                    ...
                                    if let Some(cf2) = svc_h.db.db.cf_handle("bridge_pending") {
                                        if let Ok(ser) = bincode::serialize(&op) {
                                            let _ = svc_h.db.db.put_cf(cf2, &k, &ser);
                                        }
                                    }
                                ...
                                if matches!(op.status, PendingStatus::Submitted) {
                                    ...
                                    let backoff_secs =
                                        u64::min(600, 5 * (1u64 << attempts.min(10)));
                                    ...
                                    if last == 0 || now.saturating_sub(last) >= backoff_secs {
                                        ...
                                        if let Ok(Some(latest_anchor)) =
                                            svc_h.db.get::<crate::epoch::Anchor>("epoch", b"latest")
                                        {
                                            ...
                                            .await
                                        {
                                            Ok(outcome) => {
                                                ...
                                                if let Some(cf_oc) =
                                                    svc_h.db.db.cf_handle("bridge_op_coins")
                                                {
                                                    let coins: Vec<[u8; 32]> = outcome
                                                        .spends
                                                        .iter()
                                                        .map(|s| s.coin_id)
                                                        .collect();
                                                    let ser = bincode::serialize(&coins)
                                                        .unwrap_or_default();
                                                    batch.put_cf(&cf_oc, &k, &ser);
                                                }
                                                if let Some(cf_bl) =
                                                    svc_h.db.db.cf_handle("bridge_locked")
                                                {
                                                    for c in outcome.spends.iter() {
                                                        batch.put_cf(&cf_bl, &c.coin_id, &k);
                                                    }
                                                }
                                                // Update totals and persist state
                                                if let Ok(mut st) = svc_h.state.lock() {
                                                    st.total_locked =
                                                        st.total_locked.saturating_add(op.amount);
                                                    let now = std::time::SystemTime::now()
                                                        .duration_since(std::time::UNIX_EPOCH)
                                                        .map(|d| d.as_secs())
                                                        .unwrap_or(0);
                                                    let from_addr_hex = hex::encode(op.from);
                                                    record_locked_amount(
                                                        &mut st,
                                                        &from_addr_hex,
                                                        op.amount,
                                                        now,
                                                        cfg_h.rate_window_secs,
                                                    );
                                                    if let Ok(state_ser) = bincode::serialize(&*st)
                                                    {
                                                        if let Some(cf_st) =
                                                            svc_h.db.db.cf_handle("bridge_state")
                                                        {
                                                            batch.put_cf(
                                                                &cf_st, b"state", &state_ser,
                                                            );
                                                        }
                                                    }
                                                }
                                                if let Some(cf_ev) =
                                                    svc_h.db.db.cf_handle("bridge_events")
                                                {
                                                    let ev = BridgeEvent::TokensLocked {
                                                        amount: op.amount,
                                                        sui_recipient: op.sui_recipient.clone(),
                                                        tx_hash: hex::encode(tx_hash),
                                                        op_id: String::from_utf8_lossy(&k)
                                                            .to_string(),
                                                    };
                                                    ...
                                                    let ser =
                                                        bincode::serialize(&ev).unwrap_or_default();
                                                    batch.put_cf(&cf_ev, &key, &ser);
                                                }
                                                // Remove pending on success
                                                if let Some(cf_p) =
                                                    svc_h.db.db.cf_handle("bridge_pending")
                                                {
                                                    batch.delete_cf(&cf_p, &k);
                                                }
                                                let _ = svc_h.db.db.write(batch);
                                            }
                                            Err(e) => {
                                                let is_timeout = format!("{}", e).contains(
                                                    "Timed out waiting for valid coin proof",
                                                );
                                                op.last_attempt = now;
                                                op.retry_count = op.retry_count.saturating_add(1);
                                                if !is_timeout {
                                                    op.status =
                                                        PendingStatus::Failed(format!("{}", e));
                                                }
                                                if let Some(cf3) =
                                                    svc_h.db.db.cf_handle("bridge_pending")
                                                {
                                                    let _ = svc_h.db.db.put_cf(
                                                        cf3,
                                                        &k,
                ...
                if let Some(cf_inv) = svc_h.db.db.cf_handle("bridge_invoices") {
                    let iter = svc_h
                        .db
                        .db
                        .iterator_cf(cf_inv, rocksdb::IteratorMode::Start);
                    let now_ms = std::time::SystemTime::now()
                        .duration_since(std::time::UNIX_EPOCH)
                        .map(|d| d.as_millis())
                        .unwrap_or(0) as u64;
...
fn is_protected_path(cfg: &crate::config::BridgeConfig, path: &str) -> bool {
    cfg.x402_protected_prefixes
        .iter()
        .any(|p| p == "/" || (!p.is_empty() && path.starts_with(p)))
}
...
fn build_x402_challenge(svc: &Arc<BridgeService>, resource: &str) -> Result<x402::X402Challenge> {
    ...
    let expiry_ms = std::time::SystemTime::now()
        ...
        .map(|d| d.as_millis())
        .unwrap_or(0) as u64
        + svc.cfg.x402_invoice_ttl_ms;
    let mut ch = x402::build_challenge(
        invoice_id.clone(),
        chain_id,
        recipient,
        amount,
        expiry_ms,
        svc.cfg.x402_min_confs,
        Some(resource.to_string()),
    );
    // Optionally add an EVM/facilitator method
    if let (Some(url), Some(net), Some(recv)) = (
        &svc.cfg.x402_facilitator_url,
        &svc.cfg.x402_evm_network,
        &svc.cfg.x402_evm_recipient,
    ) {
        let binding_b64 = ch
            .methods
            .get(0)
            .map(|m| m.note_binding_b64.clone())
            .unwrap_or_default();
    ...
    if let Some(cf) = svc.db.db.cf_handle("bridge_invoices") {
        let rec = X402InvoiceRecord {
            invoice_id: ch.invoice_id.clone(),
...
async fn verify_x402_any_receipt(
    svc: &Arc<BridgeService>,
    header_or_body: &str,
    ...
    if let Ok(any) = x402::decode_any_receipt_header(header_or_body) {
        match any {
            x402::X402AnyReceipt::Unchained {
                ...
                if res.is_ok() {
                    mark_invoice_used(svc, &invoice_id);
                }
                res
            }
            x402::X402AnyReceipt::Evm {
                invoice_id,
                network,
                facilitator_url,
                proof,
                ...
                let exp_url = svc
                    ...
                    .as_deref()
                    .ok_or_else(|| anyhow!("facilitator not configured"))?;
                let exp_net = svc
                    .cfg
                    .x402_evm_network
                    .as_deref()
                    .ok_or_else(|| anyhow!("evm network not configured"))?;
                if facilitator_url.trim_end_matches('/') != exp_url.trim_end_matches('/') {
                    return Err(anyhow!("facilitator url mismatch"));
                }
                if network != exp_net {
                    return Err(anyhow!("evm network mismatch"));
                }
        ...
        let receipt = match x402::decode_receipt_header(header_or_body) {
            ...
            Err(_) => serde_json::from_str::<x402::X402Receipt>(header_or_body)
                .context("invalid receipt payload")?,
        };
        verify_invoice_and_binding(
            svc,
            &receipt.invoice_id,
            resource_hint,
            receipt.amount,
        ...
        let res = verify_unchained_receipt(
            svc,
            &receipt.spend_hashes,
            receipt.amount,
            &receipt.binding_b64,
        )
        .await;
        if res.is_ok() {
            mark_invoice_used(svc, &receipt.invoice_id);
        }
        res
    }
}

fn verify_invoice_exists_and_not_expired(
    svc: &Arc<BridgeService>,
    invoice_id: &str,
    resource_hint: Option<&str>,
) -> Result<()> {
    if let Some(cf) = svc.db.db.cf_handle("bridge_invoices") {
        if let Some(bytes) = svc.db.db.get_cf(cf, invoice_id.as_bytes())? {
            if let Ok(rec) = bincode::deserialize::<X402InvoiceRecord>(&bytes) {
                ...
                if rec.used {
                    return Err(anyhow!("invoice already used"));
                }
                if let Some(res) = resource_hint {
                    if !res.is_empty() && res != rec.resource {
                        return Err(anyhow!("resource mismatch"));
                    }
                }
                return Ok(());
            }
...
fn verify_invoice_and_binding(
    ...
    let resource = if let Some(cf) = svc.db.db.cf_handle("bridge_invoices") {
        if let Some(bytes) = svc.db.db.get_cf(cf, invoice_id.as_bytes())? {
            if let Ok(rec) = bincode::deserialize::<X402InvoiceRecord>(&bytes) {
                ...
                resource_hint.unwrap_or("").to_string()
            }
        } else {
            resource_hint.unwrap_or("").to_string()
        }
    ...
    if expected != provided {
        return Err(anyhow!("binding mismatch"));
    }
    Ok(())
}
    ...
    if let Some(cf) = svc.db.db.cf_handle("bridge_invoices") {
        if let Ok(Some(bytes)) = svc.db.db.get_cf(cf, invoice_id.as_bytes()) {
            if let Ok(mut rec) = bincode::deserialize::<X402InvoiceRecord>(&bytes) {
                rec.used = true;
                rec.used_at_ms = std::time::SystemTime::now()
                    .duration_since(std::time::UNIX_EPOCH)
                    .map(|d| d.as_millis())
                    .unwrap_or(0) as u64;
                if let Ok(out) = bincode::serialize(&rec) {
                    let _ = svc.db.db.put_cf(cf, invoice_id.as_bytes(), &out);
                }
            }
        }
    }
...
async fn verify_unchained_receipt(
    svc: &Arc<BridgeService>,
    ...
    let cf = store
        .db
        .cf_handle("spend")
        .ok_or_else(|| anyhow!("spend CF missing"))?;
    ...
    for item in iter {
        ...
        if let Some(sp) = store.decode_spend_bytes_tolerant(&v) {
            ...
            if !map.contains(&tx_hash) {
                continue;
            }
            if !svc.wallet.is_output_for_me(&sp.to, &chain_id) {
                continue;
            }
    ...
    if !map.is_empty() {
        return Err(anyhow!("one or more spends not found"));
    }
    if received < amount {
        return Err(anyhow!("insufficient paid amount"));
    }
    Ok(())
}

async fn verify_evm_receipt_via_facilitator(
    _svc: &Arc<BridgeService>,
    facilitator_url: &str,
    ...
    let resp = client
        ...
        .send()
        .await
        .context("facilitator http")?;
    let status = resp.status();
    let body: serde_json::Value = resp.json().await.unwrap_or(serde_json::json!({}));
    if !status.is_success() {
        return Err(anyhow!("facilitator verify failed: status {}", status));
    }
...
async fn bridge_out_submit(svc: &Arc<BridgeService>, req: BridgeOutReq) -> Result<BridgeOutResult> {
    ...
    if !enabled {
        return Err(anyhow!("bridge disabled"));
    }
    if req.amount < min_amt || req.amount > max_amt {
        return Err(anyhow!("amount out of bounds"));
    }
    if !valid_sui_addr(&req.sui_recipient) {
        return Err(anyhow!("invalid sui recipient address"));
    }
    ...
    let op_id = format!(
        "{}",
        hex::encode(crate::crypto::blake3_hash(
            format!(
                "{}:{}:{}",
                req.amount,
                &req.sui_recipient,
    ...
    // Serialize admission and pending-op creation so concurrent requests cannot race past the cap.
    {
        let mut st = svc.state.lock().unwrap();
        let window_start = created_at.saturating_sub(rate_window_secs);
        let q = st
            .per_addr_window
            .entry(from_addr_hex.clone())
            .or_insert_with(VecDeque::new);
        prune_window(q, window_start);
        let addr_sum: u64 = q.iter().map(|&(_, a)| a).sum();
        prune_window(&mut st.global_window, window_start);
        let global_sum: u64 = st.global_window.iter().map(|&(_, a)| a).sum();
        let (pending_addr_sum, pending_global_sum) =
            svc.pending_submitted_amounts(&from_addr_hex, window_start)?;
        if addr_sum
            .saturating_add(pending_addr_sum)
            .saturating_add(req.amount)
            > per_cap
        {
            return Err(anyhow!("rate limited: per-address daily cap"));
        }
        if global_sum
            .saturating_add(pending_global_sum)
            .saturating_add(req.amount)
            > global_cap
        {
            return Err(anyhow!("rate limited: global daily cap"));
        }
        let cf = svc
            .db
            .db
    ...
    let outcome_res = svc
        .wallet
        .send_with_paycode_and_note(&vault_paycode, op.amount, &svc.net, &note)
        .await;
    let outcome = match outcome_res {
        ...
        Err(e) => {
                ...
                match svc.db.db.get_cf(cf2, op_id.as_bytes()) {
                    Ok(Some(bytes)) => {
                        if let Ok(mut po) = bincode::deserialize::<PendingBridgeOp>(&bytes) {
                            ...
                            let _ = svc.db.db.put_cf(
                                cf2,
                                op_id.as_bytes(),
                                &bincode::serialize(&po).unwrap_or_default(),
                            );
                        }
                    }
                    _ => {}
            ...
            if is_timeout {
                return Ok(BridgeOutResult::Pending {
                    op_id: op_id.clone(),
                });
            }
    ...
    // On success: wire CFs for op -> coins and coin -> op, update totals and rate windows atomically
    {
        ...
        // Update state: totals and rate windows
        {
            ...
            let now = std::time::SystemTime::now()
                ...
                .map(|d| d.as_secs())
                .unwrap_or(0);
            record_locked_amount(&mut st, &from_addr_hex, req.amount, now, rate_window_secs);
...
fn read_events(db: &Arc<Store>, limit: usize) -> Result<Vec<BridgeEvent>> {
    let cf = db
        .db
        .cf_handle("bridge_events")
        .ok_or_else(|| anyhow!("bridge_events CF missing"))?;
    let iter = db.db.iterator_cf(cf, rocksdb::IteratorMode::End);
    let mut out: Vec<BridgeEvent> = Vec::new();
    for item in iter.take(limit) {
        let (_k, v) = item?;
        if let Ok(ev) = bincode::deserialize::<BridgeEvent>(&v) {
            out.push(ev);
        }
    }
    Ok(out)
...
async fn verify_sui_burn_proof(
    svc: &Arc<BridgeService>,
    proof_bytes: &[u8],
) -> Result<VerifiedSuiBurn> {
    ...
    if !req.tx_digest.starts_with("0x") {
        return Err(anyhow!("invalid Sui digest"));
    }
    ...
    let rpc = RpcReq {
        ...
        params: vec![
            serde_json::Value::String(req.tx_digest.clone()),
            serde_json::json!({"showEffects": true, "showEvents": true}),
        ],
    };
    let cli = reqwest::Client::new();
    let resp = cli
        .post(&svc.cfg.sui_rpc_url)
        .json(&rpc)
        .send()
        .await
        .context("sui rpc http")?;
    ...
    if let Some(effects) = result.get("effects") {
        if let Some(status) = effects
            .get("status")
            .and_then(|s| s.get("status"))
            .and_then(|s| s.as_str())
        {
            if status != "success" {
                crate::metrics::BRIDGE_VERIFY_FAIL.inc();
                return Err(anyhow!("sui tx not successful"));
            }
        }
    }
    // Require checkpoint presence to ensure finalization
    if result.get("checkpoint").is_none() {
        crate::metrics::BRIDGE_VERIFY_FAIL.inc();
        return Err(anyhow!("sui tx not finalized (no checkpoint)"));
    }
    ...
    let events = result
        ...
        .or_else(|| result.get("eventsData").and_then(|v| v.as_array()))
    ...
    for ev in events {
        ...
        if !typ.starts_with(&expected_event_prefix) {
            continue;
        }
        ...
        found = Some(VerifiedSuiBurn {
            digest: req.tx_digest.clone(),
    ...
    match found {
        Some(v) => {
            crate::metrics::BRIDGE_VERIFY_OK.inc();
            Ok(v)
        }
        None => {
            crate::metrics::BRIDGE_VERIFY_FAIL.inc();
            Err(anyhow!("no matching bridge burn event"))
        }
    }
}
...
async fn bridge_in_submit(svc: &Arc<BridgeService>, body: &[u8]) -> Result<String> {
    ...
    struct InReq {
        ...
        to_paycode: String,
        sui_tx_hash: String,
        sui_burn_proof: serde_json::Value,
    }
    ...
    if !verified.digest.eq_ignore_ascii_case(&r.sui_tx_hash) {
        return Err(anyhow!("sui_tx_hash mismatch"));
    }
    // Enforce amount matches burn
    if verified.amount != r.amount {
        return Err(anyhow!("amount mismatch vs Sui burn"));
    }
    // Two-phase replay protection: check if already processed or pending
    if svc.is_sui_tx_processed(&verified.digest)? {
        crate::metrics::BRIDGE_REPLAY_ATTEMPTS.inc();
        return Err(anyhow!("sui tx already processed"));
    }
    ...
    let outcome = svc
        .wallet
        .send_with_paycode_and_note(&r.to_paycode, r.amount, &svc.net, &note)
        .await;
    ...
    // Complete the atomic update
    {
        ...
        if let Some(cf_ev) = svc.db.db.cf_handle("bridge_events") {
            ...
            let ev = BridgeEvent::TokensUnlocked {
                amount: r.amount,
                recipient: recipient_addr,
                sui_tx_hash: verified.digest.clone(),
            };
            let now_ms = std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .map(|d| d.as_millis())
...
async fn meta_transfer_submit(svc: &Arc<BridgeService>, body: &[u8]) -> Result<Vec<String>> {
    ...
    if authz.chain_id != chain_id {
        return Err(anyhow!("chain_id mismatch"));
    }

    // Validity window
    let current_epoch = svc
        .db
    ...
    let already_used = {
        let cf = svc
            .db
            .db
            .cf_handle("meta_authz_used")
            .ok_or_else(|| anyhow!("meta_authz_used CF missing"))?;
        svc.db.db.get_cf(cf, &used_key)?.is_some()
    };
    if already_used {
        return Err(anyhow!("authorization already used"));
    }
    ...
    if already_pending {
        return Err(anyhow!("authorization already pending"));
    }
    {
        let cf = svc
            .db
    ...
    for c in authz.coins.iter() {
        ...
        if preimage.len() != 32 {
            return Err(anyhow!("invalid preimage length"));
        }
        let mut p = [0u8; 32];
        p.copy_from_slice(&preimage);
    ...
    if total < authz.total_amount {
        return Err(anyhow!("authorized total not met by constructed spends"));
    }
