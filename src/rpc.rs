...
const READ_BUF_INIT: usize = 8 * 1024; // reduce initial allocations
...
pub struct ServerHelloUnsigned {
    ...
    pub kyber_ct: Vec<u8>,
    /// Server's static node Kyber public key bytes for future client encapsulation
    pub server_kyber_pk: Vec<u8>,
}
...
impl Codec for RpcCodec {
        ...
        T: AsyncRead + Unpin + Send,
    {
        ...
        // Reserve exactly the announced size; avoid repeated growth
        let mut buf = Vec::with_capacity(len.max(READ_BUF_INIT));
        buf.resize(len, 0);
...
pub fn seal_response(
    ...
    // Server's static Kyber public key bytes for client to pin
    server_kyber_pk: Vec<u8>,
    suites: &str,
    local_peer_id: &str,
    remote_peer_id: &str,
